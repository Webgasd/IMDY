...
public class SupervisionEnterpriseServiceImpl implements SupervisionEnterpriseService {
    ...
    @Override
    public EnterpriseParam getById(int id) throws ParseException {
        ...
        if (enterpriseParam.getBusinessTermStart() != null && !enterpriseParam.getBusinessTermStart().equals("") && enterpriseParam.getBusinessTermStart().contains("CST")) {
            String formatStr = new SimpleDateFormat("yyyy-MM-dd").format((Date) sdf.parse(enterpriseParam.getBusinessTermStart()));
            enterpriseParam.setBusinessTermStart(formatStr);
        }

        if (enterpriseParam.getBusinessTermEnd() != null && !enterpriseParam.getBusinessTermEnd().equals("") && enterpriseParam.getBusinessTermEnd().contains("CST")) {
            String formatStr = new SimpleDateFormat("yyyy-MM-dd").format((Date) sdf.parse(enterpriseParam.getBusinessTermEnd()));
            enterpriseParam.setBusinessTermEnd(formatStr);
        }
        ...
        if (type == 7) {
            try {
                ...
                for (int j = 0; j < rowNumber; j++) {
                    ...
                    if ((row.getCell(9) != null) && row.getCell(9).getCellType() != CellType.BLANK && (row.getCell(9).getCellType() != CellType.NUMERIC || !DateUtil.isCellDateFormatted(row.getCell(9)))) {
                        errorList.add("企业基本信息表主表 第" + a + "行" + titleRow.getCell(9).toString() + "不是日期类型");
                    }
                    if ((row.getCell(10) != null) && row.getCell(10).getCellType() != CellType.BLANK && (row.getCell(10).getCellType() != CellType.NUMERIC || !DateUtil.isCellDateFormatted(row.getCell(10)))) {
                        errorList.add("企业基本信息表主表 第" + a + "行" + titleRow.getCell(10).toString() + "不是日期类型");
                    }
                    if ((row.getCell(11) != null) && row.getCell(11).getCellType() != CellType.BLANK && (row.getCell(11).getCellType() != CellType.NUMERIC || !DateUtil.isCellDateFormatted(row.getCell(11)))) {
                ...
                if (!errorList.isEmpty()) {
                    String str1 = org.apache.commons.lang.StringUtils.join(errorList, ", ");
                    throw new BusinessException(EmBusinessError.PARAMETER_VALIDATION_ERROR, str1);
                }
                for (int j = 0; j < rowNumber; j++) {
                    if (j == 0) {
                        continue;//标题行
                    ...
                    //如果营业期限至为空，则设置长期标志位
                    if ((ExcalUtils.handleDateXSSF(row.getCell(9)) + "") != null &&
                            !(ExcalUtils.handleDateXSSF(row.getCell(9)) + "") .equals("") &&
                            !(ExcalUtils.handleDateXSSF(row.getCell(9)) + "") .equals("null"))
                    {
                        supervisionEnterprise.setBusinessTermStart((new SimpleDateFormat("yyyy-MM-dd").format(ExcalUtils.handleDateXSSF(row.getCell(9)))));
                    }
                    if ((ExcalUtils.handleDateXSSF(row.getCell(10)) + "") == null ||
                    ...
                    if (!supervisionEnterprise.getIdNumber().equals("")) {
                        if (enterpriseIdMap.get(supervisionEnterprise.getIdNumber()) != null && enterpriseIdMap.get(supervisionEnterprise.getIdNumber())!=0) {
                            ...
                            if (supervisionEnterprise1.getCantactWay() != null && !supervisionEnterprise1.getCantactWay().equals("")) {
                                supervisionEnterprise.setCantactWay(supervisionEnterprise1.getCantactWay());
                            }
                            supervisionEnterprise.setId(id);
                            supervisionEnterpriseMapper.updateByPrimaryKeySelective(supervisionEnterprise);
                            updateNumber++;
